...
def fetch_town_houses_list(town):
    """Krok 1: Pobiera listę domków z miasta (JSON) w ułamku sekundy."""
...
def scrape_house_direct_tibia(house_id, house_name, town, now_str):
    """Krok 2: Bezpośredni scraping podstrony domku na Tibia.com (HTML)."""
    if not house_id:
        return None
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Szukamy wzorca licytacji w HTML Tibia.com
            match = re.search(r"highest bid so far is\s+([\d,]+)\s+gold\s+and has been submitted by\s+<a[^>]*>([^<]+)</a>", html, re.IGNORECASE)
            if match:
                gold_str = match.group(1).replace(",", "").replace(".", "").strip()
...
def get_active_houses_to_track():
    """Tworzy listę licytowanych domków na serwerze."""
    candidates = []
    for town in TOWNS:
...
def scan_houses_direct(houses_to_track, now_str):
    """Odpytuje bezpośrednio Tibia.com równolegle w wielu wątkach."""
    bids = []
    ...
    if is_sniper_time(now_utc):
        ...
        for b in active_bids:
            ...
            if key not in last_bids or last_bids[key] != curr_state:
                last_bids[key] = curr_state
                new_to_save.append(b)
                print(f"  [+] NOWA OFERTA (Direct Tibia.com): {b['House Name']} ({b['Town']}) -> {b['Player Nick']} ({b['Gold Amount']} gp)", flush=True)

        if new_to_save:
            append_and_clean_csv(new_to_save)
            print(f"Zapisano {len(new_to_save)} nowych ofert.", flush=True)
        else:
