...
def scrape_house_direct_tibia(house_id, house_name, town, now_str):
  ...
  try:
    ...
    if resp.status_code == 200:
      ...
      # 1. Wyciąganie kwoty złota
      bid_match = re.search(
          r"highest bid so far is\s+([\d,.]+)\s+gold", raw_html, re.IGNORECASE
      )
      if not bid_match:
        # Alternatywny wzorzec w tabeli widoku
        bid_match = re.search(
            r"(?:current|highest)\s+bid:\s*</td>\s*<td>\s*([\d,.]+)",
            raw_html,
            re.IGNORECASE,
        )

      # 2. Wyciąganie nicku gracza
...
def get_active_houses_to_track():
  ...
  # Zabezpieczenie: Alai Flats, Flat 25 (Thais) zawsze na liście
  found_alai = any("Alai Flats" in str(c[1]) for c in candidates)
  if not found_alai:
  ...
  if is_sniper_time(now_utc):
    ...
    while time.time() - start_t < 420:
      ...
      for b in active_bids:
        ...
        if key not in last_bids or last_bids[key] != curr_state:
          ...
          print(
              f"[{b['Timestamp_UTC']}] DIRECT PRZEBICIE: {b['House Name']} ->"
    ...
    for b in active_bids:
      ...
      if key not in last_bids or last_bids[key] != curr_state:
        ...
        new_to_save.append(b)
        print(
            f"  [+] NOWA OFERTA (Direct Tibia.com): {b['House Name']} ({b['Town']}) ->"
            f" {b['Player Nick']} ({b['Gold Amount']} gp)",
            flush=True,
        )
