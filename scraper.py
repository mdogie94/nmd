            ...
            def find_fields(obj):
                ...
                if isinstance(obj, dict):
                    for k, v in obj.items():
                        kl = k.lower()
                        if any(x in kl for x in ["bidder", "current_bidder", "highest_bidder", "character"]):
                            if v and isinstance(v, str) and v.strip().lower() not in ["", "none", "null", "false", "brak ofert"]:
                                if not bidder: bidder = v.strip()
                        if any(x in kl for x in ["bid", "current_bid", "highest_bid", "gold"]):
                            if v is not None and not isinstance(v, (dict, list)):
                                clean = re.sub(r"[^\d]", "", str(v))
                                if clean:
                                    val = int(clean)
                                    if val > gold: gold = val
                        find_fields(v)
                elif isinstance(obj, list):
                    for item in obj: find_fields(item)
...
def check_house(house_id, house_name, town, now_str):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Wyszukiwanie kwoty
            bids = re.findall(r"(?:highest bid|current bid|bid so far is|auctioned.*?is)\s*[:]?\s*<b>?([\d,.]+)\s*gold", text, re.IGNORECASE)
            if not bids:
                bids = re.findall(r"([\d,.]+)\s+gold\s+(?:and has been submitted|has been submitted)", text, re.IGNORECASE)
            
            # Wyszukiwanie gracza
            nicks = re.findall(r"submitted by\s+<a[^>]*>([^<]+)</a>", text, re.IGNORECASE)
            if not nicks:
                nicks = re.findall(r"submitted by\s+<b>([^<]+)</b>", text, re.IGNORECASE)
            if not nicks:
                nicks = re.findall(r"submitted by\s+([A-Z][a-zA-Z\s]{1,25})(?:\.|\s*and|\s*\(|<|$)", text, re.IGNORECASE)

            if bids and nicks:
                gold_str = re.sub(r"[^\d]", "", bids[0])
                bidder = nicks[0].strip()
                if bidder and gold_str and int(gold_str) > 0:
                    print(f"  [HTML HIT] {house_name} -> {bidder} ({gold_str} gp)", flush=True)
                    return {
    ...
    if is_sniper_time(now_utc):
        ...
        while time.time() - start_t < 420:
            ...
            for b in active_bids:
                ...
                if key not in last_bids or last_bids[key] != curr_state:
                    ...
                    print(f"[{b['Timestamp_UTC']}] PRZEBICIE: {b['House Name']} -> {b['Player Nick']} ({b['Gold Amount']} gp)", flush=True)
        ...
        for b in active_bids:
            ...
            if key not in last_bids or last_bids[key] != curr_state:
                last_bids[key] = curr_state
                new_to_save.append(b)
                print(f"  [+] NOWY WPIS: {b['House Name']} ({b['Town']}) -> {b['Player Nick']} ({b['Gold Amount']} gp)", flush=True)

        if new_to_save:
            append_and_clean_csv(new_to_save)
            print(f"Pomyślnie zapisano {len(new_to_save)} nowych rekordów do CSV.", flush=True)
        else:
