rom concurrent.futures import ThreadPoolExecutor, as_completed
...
    # Konwertujemy Timestamp_UTC do poprawnego sortowania chronologicznego
...
  # Parsowanie daty i usuwanie rekordów starszych niż 7 dni
...
  # Sortowanie od najnowszych do najstarszych (najnowsze na górze)
  df_cleaned = df_cleaned.sort_values(by="_dt", ascending=False)
  df_cleaned = df_cleaned[cols]

  # Zapis całego zaktualizowanego pliku
...
  if is_sniper_window:
    print(
        "=== TRYB SNAJPERA: monitoring co 3s do 10:00:05 CEST ===", flush=True
    )
    while True:
...
          updates.append(r)
          print(
              f"  [+] NOWE PRZEBICIE: {r['House Name']} ({r['Town']}) ->"
...
        updates.append(r)
        print(
            f"  [+] RZECZYWISTA ZMIANA: {r['House Name']} ({r['Town']}) ->"
            f" {r['Player Nick']} ({r['Gold Amount']} gp)",
            flush=True,
        )

    # Zawsze wywołujemy zapis, by odświeżyć 7-dniowe okno i posortować tabelę
    update_and_cleanup_csv(updates)
...
    else:
      print(
          "Brak zmian w ofertach. Historia wyczyszczona i posortowana.",
          flush=True,
      )
